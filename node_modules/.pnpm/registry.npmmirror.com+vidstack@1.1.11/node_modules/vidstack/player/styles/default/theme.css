...
@media (hover: hover) and (pointer: fine) {
  :where(.vds-menu-items) :where([role='menuitem']:hover, [role='menuitemradio']:hover) {
    cursor: pointer;
    background-color: var(--media-menu-item-hover-bg, rgb(245 245 245 / 0.08));
  }
}
...
:where(.vds-radio[aria-checked='true'] .vds-radio-check) {
...
:where(.vds-menu[data-keyboard]) .vds-chapters-radio-group:focus-within {
