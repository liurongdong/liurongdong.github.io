import { H as Host, e as effect, f as setAttribute, h as signal, j as computed, C as Component, l as listenEvent, k as isString, u as useState } from './vidstack-4af9f92b.js';
...
import { C as ControlsGroup, T as TooltipTrigger, M as MenuItem } from './vidstack-bbf44757.js';
import { o as observeActiveTextTrack } from './vidstack-b3a8b421.js';
import { c as cloneTemplateContent, a as createTemplate, r as requestScopedAnimationFrame, b as cloneTemplate } from './vidstack-e557f714.js';
...
class MediaPosterElement extends Host(HTMLElement, Poster) {
  ...
  onSetup() {
    ...
    this.$state.img.set(this.Df);
  }
  onConnect() {
    const { src, alt } = this.$state, { crossorigin } = this.v.$state;
    if (this.Df.parentNode !== this) {
      this.prepend(this.Df);
    }
    effect(() => {
      setAttribute(this.Df, "src", src());
      setAttribute(this.Df, "alt", alt());
      setAttribute(this.Df, "crossorigin", crossorigin());
    });
  }
}
...
class MediaThumbnailElement extends Host(HTMLElement, Thumbnail) {
  ...
  onConnect() {
    const { src } = this.$state, { crossorigin } = this.v.$props;
    if (this.Df.parentNode !== this) {
      this.prepend(this.Df);
    }
    effect(() => {
...
class MediaChapterTitleElement extends Host(HTMLElement, ChapterTitle) {
  constructor() {
    ...
    this.Ff = signal("");
    this.Gf = computed(() => {
      const { title, started } = this.v.$state;
      return started() ? this.Ff() || title() : title();
    });
  }
  static {
  ...
  onConnect() {
    observeActiveTextTrack(this.v.textTracks, "chapters", (track) => {
      if (!track) {
        this.Ff.set("");
        return;
      }
      const onCueChange = () => {
        ...
        this.Ff.set(activeCue?.text || "");
      };
      onCueChange();
      listenEvent(track, "cue-change", onCueChange);
    });
    effect(() => {
      this.textContent = this.Gf();
    });
  }
}
...
class MediaSliderThumbnailElement extends MediaThumbnailElement {
  ...
  kf() {
    ...
    this.time = this.Fc.pointerRate() * duration();
  }
}
...
class MediaSliderVideoElement extends Host(HTMLElement, SliderVideo) {
  ...
  onSetup() {
    this.v = useMediaContext();
    this.$state.video.set(this._);
  }
  onConnect() {
    const { crossorigin, canLoad } = this.v.$state, { src } = this.$state;
    if (this._.parentNode !== this) {
      this.prepend(this._);
    }
    effect(() => {
      setAttribute(this._, "src", src());
      setAttribute(this._, "crossorigin", crossorigin());
      setAttribute(this._, "preload", canLoad() ? "auto" : "none");
    });
  }
  Cf() {
...
class MediaSliderChaptersElement extends Host(HTMLElement, SliderChapters) {
  ...
  If() {
    if (!this.Hf)
      return;
    const elements = cloneTemplate(this.Hf, this.cues.length || 1);
