import { e, c, t, l, D, x, o } from './vidstack-cee66f8f.js';
import { j as computed, e as effect, K as animationFrameThrottle, o as onDispose, z as isFunction, L as unwrap, k as isString, l as listenEvent } from './vidstack-4af9f92b.js';
import { b as useDefaultLayoutContext, u as useMediaContext, K as useDefaultLayoutLang } from './vidstack-e5068f63.js';
...
class SignalDirective extends c {
  ...
  Zg() {
    if (!this.Xg)
      return;
    this.Db = effect(this.Uc.bind(this));
  }
  Uc() {
    {
      this.setValue(this.Xg?.());
    }
  }
}
...
class SlotObserver {
  constructor(_root, _callback) {
    ...
    this.Pb = animationFrameThrottle(this.Y.bind(this));
    this._g = _root;
    this.Wg = _callback;
  }
  connect() {
    this.Y();
    const observer = new MutationObserver(this.Pb);
    observer.observe(this._g, { childList: true });
  ...
  Y() {
    for (const slot of this._g.querySelectorAll("slot")) {
      if (slot.hasAttribute("name"))
        this.elements.add(slot);
    }
    this.Wg(this.elements);
  }
}
...
class SlotManager {
  constructor(_root) {
    this.Pb = animationFrameThrottle(this.Y.bind(this));
    this._g = _root;
    this.slots = new SlotObserver(_root, this.Y.bind(this));
  }
  connect() {
    this.slots.connect();
    this.Y();
    const mutations = new MutationObserver(this.Pb);
    mutations.observe(this._g, { childList: true });
    onDispose(() => mutations.disconnect());
  }
  Y() {
    for (const node of this._g.children) {
      if (node.nodeType !== 1)
        continue;
...
class IconsLoader {
  constructor(_root) {
    ...
    this.Ij = false;
    this._g = _root;
    this.slots = new SlotObserver(_root, this.Jj.bind(this));
  }
  connect() {
    this.slots.connect();
    onDispose(this.disconnect.bind(this));
  ...
  disconnect() {
    for (const { slot } of this.Kj()) {
      slot.textContent = "";
    }
  }
  *Kj() {
    for (const iconName of Object.keys(this.Hj)) {
      const slotName = `${iconName}-icon`;
      for (const slot of this.slots.elements) {
        if (slot.name !== slotName)
          continue;
        yield { icon: this.Hj[iconName], slot };
      }
    }
  }
  Jj() {
    if (!this.Ij)
      return;
    for (const { icon, slot } of this.Kj()) {
...
class LayoutIconsLoader extends IconsLoader {
  ...
  Cj() {
    let node = this._g.parentElement;
